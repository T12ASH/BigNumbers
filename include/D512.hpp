...
class D512 {
    ...
    // Впомогательные методы (Парсинг строк) _______________

    /**
     * @brief Умножает текущее число на 10
     * @throw std::overflow_error если результат не влезает в 512 бит
     */
    void multiplyBy10();
    
    /**
     * @brief Прибавляет однозначное число (0-9)
     * @param digit цифра для прибавления
     * @throw std::overflow_error если результат не влезает в 512 бит
     */
    void addDigit(uint64_t digit);
    
    /**
     * @brief Умножает два 64-битных числа, получая 128-битный результат
     * @param a первый множитель
     * @param b второй множитель
     * @param low младшие 64 бита результата
     * @param high старшие 64 бита результата
     */
    static void multiply64(uint64_t a, uint64_t b, uint64_t& low, uint64_t& high);
